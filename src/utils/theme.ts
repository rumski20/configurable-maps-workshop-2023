 ...
 */
export const getThemeUrl = (theme: ThemeType) =>
  `https://js.arcgis.com/${arcgisVersion}/esri/themes/${theme}/main.css`;
...
export const loadEsriCss = () => {
  ...
  for (const theme of ["light", "dark"] as ThemeType[]) {
    const targetRef =
      theme === "light" ? lightThemeStyleLink : darkThemeStyleLink;
    ...
    if (!targetRef.value) {
      ...
      console.log(`created esri theme link with id: "${link.id}"`);
      targetRef.value = link;
      // check if this is not the active theme
      if (appStore.theme !== theme) {
        deactivateStyleLink(link);
        console.log(`deactivated esri style link for theme: "${theme}"`);
      }
    }
  }
};
...
export const setBootstrapTheme = (theme?: ThemeType) => {
  ...
  console.log(`updated the bootstrap theme to "${theme}"`);
};
